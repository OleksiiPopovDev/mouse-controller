...
from ..utils.helpers import validate_coordinates, get_screen_bounds
...
class MouseMover:
    ...
    def __init__(self, failsafe: bool = True, pause: float = 0.1):
        ...
        self.logger.info(f"MouseMover initialized. Screen size: {self.screen_width}x{self.screen_height}")
    ...
    def move_relative(self, x_offset: int, y_offset: int, duration: float = 1.0) -> bool:
        """
    ...
    def move_in_circle(self, center_x: int, center_y: int, radius: int = 100, 
                      steps: int = 50, clockwise: bool = True) -> bool:
        """
        ...
        """
        try:
            self.logger.info(f"Moving in circle: center ({center_x}, {center_y}), radius {radius}")
    ...
    def move_in_square(self, start_x: int, start_y: int, size: int = 200) -> bool:
        ...
        """
        try:
            self.logger.info(f"Moving in square: start ({start_x}, {start_y}), size {size}")
            ...
            for point in points:
                if validate_coordinates(point[0], point[1], self.screen_width, self.screen_height):
    ...
    def shake_cursor(self, duration: float = 2.0, intensity: int = 50) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Shaking cursor: duration {duration}s, intensity {intensity}")
            ...
            while time.time() - start_time < duration:
                ...
                if validate_coordinates(new_x, new_y, self.screen_width, self.screen_height):

...
def wait_with_countdown(seconds: int, message: str = "Operation will start in"):
    ...
    print(f"\r🚀 Starting execution!", flush=True)
    ...
    while True:
        try:
            ...
            if choice == "0":
                ...
                print(f"🔄 Moving in circle (radius: {radius}, center: {center_x}, {center_y})")
                wait_with_countdown(3)
                safe_execute(mover.move_in_circle, center_x, center_y, radius, 100, clockwise)
                ...
                points = pattern_gen.generate_star_points(center_x, center_y, outer_radius, inner_radius, star_points)
                ...
                points = pattern_gen.generate_spiral_points(center_x, center_y, max_radius, turns, 50)
                ...
                points = pattern_gen.generate_sine_wave_points(start_x, center_y, length, amplitude, frequency, 100)
                print(f"〰️ Moving in sine wave (length: {length}, amplitude: {amplitude})")
                wait_with_countdown(3)
                safe_execute(mover.move_smooth_path, points, 0.05)
                ...
                points = pattern_gen.generate_heart_points(center_x, center_y, size, 100)
                wait_with_countdown(3)
                safe_execute(mover.move_smooth_path, points, 0.05)
                ...
                points = pattern_gen.generate_figure_eight(center_x, center_y, width, height, 100)
                wait_with_countdown(3)
                safe_execute(mover.move_smooth_path, points, 0.05)
                ...
                points = pattern_gen.generate_random_walk(current_x, current_y, steps, max_step)
